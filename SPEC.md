...
- **單一方向**：買進與賣空條件同時成立時（近 10 日既有長下影也有長上影），**只輸出一個方向**：保留「最近一次命中影線」的方向；命中日相同則比次數、再比平均影線占比。
...
同一檔若兩根分別大跌與大漲，**只保留較晚那根（今日優先）的方向**，不輸出買賣各一筆。`metrics` 另寫入 `yesterday_open`、`yesterday_close`、`today_open`、`today_close` 供驗證。

同一檔股票可同時命中多個策略（含日K 與策略選股），會各寫一筆。
**同一檔、同一策略、同一選股日只輸出一個方向**（買進或賣空擇一）。若該策略買賣條件同時成立：S2 保留最近一次命中影線的方向；其餘策略以今日漲跌為準（上漲留買進、下跌留賣空）。
...
網頁「策略選股」分頁使用 D1–D6。這六條是依主流短線教材與可公開文獻，挑出**期望值較高、且可用日K 排出短線觀察名單**的選股法。掃描時同樣通過 3.0 共用門檻。各策略當日同一檔只輸出一個方向（買進或賣空擇一）。
...
- 買進與賣空互斥（綠K／紅K）；極端情況同時成立時只保留收盤方向。
